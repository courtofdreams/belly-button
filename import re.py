...
class GoogleMAPService:
    """
    Google Maps Places pipeline encapsulating the full flow:
    Search → Parse → Sentiment Analysis → Heuristic Scoring → Recommendation

    No OpenAI / LLM dependency — all ranking is rule-based.
    All data is plain dicts / list[dict] — no pandas dependency.
    """
    ...
    # -------------------------------------------------------------------------
    # 2. Heuristic Recommendation (no LLM)
    # -------------------------------------------------------------------------

    def _compute_score(self, place: dict) -> float:
        """
    ...
    def recommend_restaurants(self, restaurants: dict, top_k: int = 4) -> list[dict]:
        ...
        return sorted(filtered, key=self._compute_score, reverse=True)[:top_k]
    ...
    # -------------------------------------------------------------------------
    # 5. Look Up Restaurant by Name (post-OpenAI enrichment)
    # -------------------------------------------------------------------------
    ...
    def enrich_openai_results(
        ...
        for restaurant in openai_results:
            ...
            print(f"\nEnriching OpenAI result of {name}")
    ...
    def search_restaurant_info(
        ...
        """
        Search for restaurant information using the Google Custom Search JSON API.
        Complements Places API data with web snippets (articles, blogs, review sites).

        Args:
            cse_api_key:  Your Google API key (may differ from self.api_key if you
                          use separate keys per product — pass explicitly to be safe).
            cse_id:       Your Programmable Search Engine ID (cx parameter).
            query:        Free-text restaurant query
                          (e.g. "Best Ice Cream San Francisco site:yelp.com OR site:eater.com").
            max_results:  Number of results to return, 1–10 (API max per request is 10).

        Returns:
            list[dict], one entry per search result, each containing:
                - title       (str)  Page title
                - link        (str)  URL
                - snippet     (str)  Short text excerpt from the page
                - source      (str)  Domain extracted from the URL
                - query       (str)  The original query (for traceability)
                - name        (str)  Best-guess restaurant name parsed from the title
                - address     (str)  Address snippet if detectable in the snippet, else ""
                - rating      (str)  Rating string if detectable in the snippet, else ""
                - description (str)  Full snippet re-exposed for downstream use
        """
        params = {
            "key": cse_api_key,
            "cx":  cse_id,
            "q":   query,
            "num": min(max_results, 10),  # API hard limit is 10 per request
        }
        ...
        for item in items:
            ...
            # Best-guess restaurant name: take the part of the title before
            # the first separator character ("|", "-", "–", "·", "—")
            name_part = re.split(r"[|\-–·—]", title)[0].strip()

            # Lightweight field extraction from snippet text
            address = ""
            rating  = ""

            # Rating pattern: e.g. "4.5", "4.5/5", "4.5 stars", "Rated 4.5"
            rating_match = re.search(
                r"(?:rated?\s*)?(\d\.\d)\s*(?:/\s*5|stars?)?", snippet, re.IGNORECASE
            )
            if rating_match:
                rating = rating_match.group(1)

            # Address pattern: look for a number followed by a street name keyword
            address_match = re.search(
                r"\d{1,5}\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\s+"
                r"(?:St|Ave|Blvd|Dr|Rd|Ln|Way|Pl|Court|Ct|Terrace|Ter|"
                r"Street|Avenue|Boulevard|Drive|Road|Lane|Place)[\w\s,]*",
                snippet,
            )
            if address_match:
        ...
        print(f"Google Custom Search: {len(results)} results for '{query}'")
        return results
    ...
    def run_pipeline(self, query: str, max_results: int = 10, top_k: int = 4) -> list[dict]:
        ...
        print(f"\n── Scoring & ranking {len(results)} places ──")
